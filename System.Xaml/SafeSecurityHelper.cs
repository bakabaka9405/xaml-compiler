...
internal static class SafeSecurityHelper
{
	...
	internal static Assembly GetLoadedAssembly(AssemblyName assemblyName)
	{
		...
		for (int num = assemblies.Length - 1; num >= 0; num--)
		{
			...
			if (string.Compare(assemblyName2.Name, assemblyName.Name, ignoreCase: true, TypeConverterHelper.InvariantEnglishUS) == 0 && (version == null || version.Equals(version2)) && (cultureInfo == null || cultureInfo.Equals(cultureInfo2)) && (publicKeyToken == null || IsSameKeyToken(publicKeyToken, publicKeyToken2)))
			{
